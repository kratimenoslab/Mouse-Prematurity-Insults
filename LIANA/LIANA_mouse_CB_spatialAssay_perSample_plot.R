...
prepare_liana_condition_plot_data <- function(
  ...
  interaction_ranked <- cond_present_both %>%
    ...
    arrange(min_aggregate_rank, desc(mean_sca_LRscore))
  ...
  if (selection_method == "top20_total") {
    ...
    selected_interactions <- interaction_ranked %>%
      ...
      arrange(min_aggregate_rank, desc(mean_sca_LRscore), .by_group = TRUE) %>%
...
for (cond in cond_order) {
  ...
  ggsave(
    filename = paste0(outputDir, prefix, "liana_dotplot_", cond, "_top20_anyPCGC_presentBothSamples_minRank_avgMagSpecSamples.pdf"),
    plot = p,
    width = 14,
    height = 10,
    units = "in"
...
write.csv(
  ...
  paste0(outputDir, prefix, "selected_interactions_top20_anyPCGC_presentBothSamples_minRank_avgMagSpecSamples.csv"),
  row.names = FALSE
)
...
ggsave(
  filename = paste0(outputDir, prefix, "liana_dotplot_combined_top20_anyPCGC_presentBothSamples_minRank_avgMagSpecSamples.pdf"),
...
for (cond in cond_order) {
  ...
  ggsave(
    filename = paste0(outputDir, prefix, "liana_dotplot_", cond, "_top10_eachDirection_presentBothSamples_minRank_avgMagSpecSamples.pdf"),
    plot = p,
    width = 14,
    height = 10,
    units = "in"
...
write.csv(
  ...
  paste0(outputDir, prefix, "selected_interactions_top10_eachDirection_presentBothSamples_minRank_avgMagSpecSamples.csv"),
  row.names = FALSE
)
...
ggsave(
  filename = paste0(outputDir, prefix, "liana_dotplot_combined_top10_eachDirection_presentBothSamples_minRank_avgMagSpecSamples.pdf"),
